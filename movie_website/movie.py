...
class Movie( ) :
	def __init__( self, movie_name, movie_storyline, movie_poster_url, movie_trailer_url) :
		...
		self.movie_storyline_object = movie_storyline
		self.movie_poster_url_object = movie_poster_url
		self.movie_trailer_url_object = movie_trailer_url
...
def create_movie_tiles_content( movies ) :
	content = ''
	for movie in movies :
		youtube_id_match = re.search( r'(?<=v=)[^&#]+', movie.movie_trailer_url_object )
		youtube_id_match = youtube_id_match or re.search(r'(?<=be/)[^&#]+', movie.trailer_youtube_url)
        trailer_youtube_id = (youtube_id_match.group(0) if youtube_id_match else None)

        content = content + movie_tile_content.format( movie_title = movie.movie_name_object, poster_image_url = movie.movie_poster_url_object, trailer_youtube_id = movie.movie_trailer_url_object )
	print "content"
	print content
	print "hello"
	return content
...
def open_movies_page( movies ) :
	...
	output_file = open( 'webpage_for_movies_website.html', 'w')
	rendered_content = main_page_content.format( movie_tiles = create_movie_tiles_content( movies ) )

	output_file.write( main_page_head + rendered_content )
	
	output_file.close( )
	output_file = open( 'webpage_for_movies_website.html', 'r')
